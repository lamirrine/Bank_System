package view;

import javax.swing.*;
import java.awt.*;

public class LevantamentoPanel extends JPanel {
    private JTextField txtValor;
    private JPasswordField txtPin;
    private JButton btnConfirm;
    private JLabel lblStatus;

    public LevantamentoPanel() {
        setLayout(new BorderLayout(12,12));
        setBackground(new Color(0xF5F7FA));
        setBorder(BorderFactory.createEmptyBorder(8,8,8,8));
        init();
    }

    private void init() {
        JPanel form = new JPanel(new GridBagLayout());
        form.setBackground(new Color(0xF5F7FA));
        GridBagConstraints c = new GridBagConstraints();
        c.insets = new Insets(10,10,10,10);

        c.gridx=0;c.gridy=0; form.add(new JLabel("Valor (MZN):"), c);
        c.gridx=1; txtValor = new JTextField(12); form.add(txtValor, c);

        c.gridy=1; c.gridx=0; form.add(new JLabel("PIN (4 dígitos):"), c);
        c.gridx=1; txtPin = new JPasswordField(6); form.add(txtPin, c);

        add(form, BorderLayout.CENTER);

        JPanel bottom = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        bottom.setOpaque(false);
        btnConfirm = new JButton("Confirmar Levantamento");
        lblStatus = new JLabel(" ");
        bottom.add(lblStatus);
        bottom.add(btnConfirm);
        add(bottom, BorderLayout.SOUTH);
    }

    public String getValorText() { return txtValor.getText().trim(); }
    public String getPin() { return new String(txtPin.getPassword()).trim(); }
    public void setStatus(String s) { lblStatus.setText(s); }
    public void addConfirmListener(java.awt.event.ActionListener l) { btnConfirm.addActionListener(l); }
}
